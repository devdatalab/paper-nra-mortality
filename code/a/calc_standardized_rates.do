...
/**************************************************************/
global std $out/outfiles

/*************************************/
/* prep standardized u.s. population */
/*************************************/
insheet using  $mortality/std-pop-us-2000-1.csv, clear names
ren uss pop
drop v3
drop if mi(age)
...
insheet using  $mortality/std-pop-us-2000-5.csv, clear names
keep age std2000tot
ren std pop
...
/****************************************************************************/
use $mdata/mort_changes_all, clear
...
// /* get average age */
// use $mortality/outfiles/nchs/appended_rank_mortgranage, clear
// 
  ...
  foreach sex in 2 1 {
    ...
    forval ed = 1/4 {
      foreach cause in a c d h o t {
        ...
        if "`cause'" == "t" {
          store_constant using $out/mort_paper_stats.csv, value("`s2'") desc("`race`race'' `sex`sex'' ed`ed'") category("Age-Adjusted all-age Mortality Change %") format(string)
        }
      }
    }
  }
