/*
File:	cepr_acs_read_2017.do
Date:	May 30, 2019, CEPR ACS Version 1.4
	Apr 16, 2020, CEPR ACS Version 1.5
Desc:	Reads and pre-processes raw American Community Survey 1-year PUMS data from 2017
*/

capture program drop rd2017

if $statanew==1 {
program define rd2017
version 9.0
...
compress
cd "$working"
save "p_2017_temp_a.dta", replace
...
compress
cd "$working"
save "p_2017_temp_b.dta", replace
...
compress
cd "$working"
save "h_2017_temp_a.dta", replace
...
compress
cd "$working"
save "h_2017_temp_b.dta", replace
...
capture program drop com2017
program define com2017
version 9.0

/* append p files */

cd "$working"
use "p_2017_temp_a", clear
append using "p_2017_temp_b"
sort serialno sporder
save "p_2017_temp_ab", replace
cd "$raw_data"
sort serialno sporder
label data "Raw 2017 ACS person level data, converted to Stata format"
save "acs_2017_P_raw", replace
...
cd "$working"
use "h_2017_temp_a", clear
append using "h_2017_temp_b"
sort serialno
save "h_2017_temp_ab", replace
cd "$raw_data"
sort serialno
label data "Raw 2017 ACS household level data, converted to Stata format"
save "acs_2017_H_raw", replace
...
cd "$working"
use "h_2017_temp_ab", clear
merge serialno using "p_2017_temp_ab"
...
compress
cd "$working"
save "acs_2017_pre_all", replace

cd "$working"
!rm "p_2017_temp_a.dta"
!rm "p_2017_temp_b.dta"
!rm "p_2017_temp_ab.dta"
!rm "h_2017_temp_a.dta"
!rm "h_2017_temp_b.dta"
!rm "h_2017_temp_ab.dta"
...
/* 
Copyright 2020 CEPR, John Schmitt, Cherrie Bucknor, Hye Jin Rho, Hayley Brown
